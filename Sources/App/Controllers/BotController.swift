...
class BotController {
    ...
    private func getKeyboardChat(for userId: Int64) -> InlineKeyboardAttachmentRequest {
        //let buttons = [[LinkButton(text: "Google", url: GoogleAuthController().getUrlForAuth(for: userId))]]
        let buttons = [[ChatButton(text: "Email", chatTitle: "Email", chatDescription: "hkjdghfhjkdhfk")]]
        let keyboardPayload = InlineKeyboardAttachmentRequestPayload(buttons: buttons)
        let keyboard = InlineKeyboardAttachmentRequest(payload: keyboardPayload)
        return keyboard
    }
    ...
    private func sendKeyboardChat(to chatId: Int64, and userId: Int64) throws {
        let keyboard = getKeyboardChat(for: userId)
        let message = NewMessageBody(with: "Open chat:", with: [keyboard])
        let json = try JSONEncoder().encode(message)
